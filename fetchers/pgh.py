...
def collect():
    bus = main("https://truetime.portauthority.org/gtfsrt-bus/vehicles")
    bustrip = main("https://truetime.portauthority.org/gtfsrt-bus/trips")
    train = main("https://truetime.portauthority.org/gtfsrt-train/vehicles")
    traintrip = main("https://truetime.portauthority.org/gtfsrt-train/trips")
    
    fm_str = json_format.MessageToJson(bus)
    fm_str2 = json_format.MessageToJson(bustrip)
    fm_str3 = json_format.MessageToJson(train)
    fm_str4 = json_format.MessageToJson(traintrip)

    write_to_file("data/prt/vehicles-bus.txt", fm_str)
    write_to_file("data/prt/trips-bus.txt", fm_str2)
    write_to_file("data/prt/vehicles-train.txt", fm_str3)
    write_to_file("data/prt/trips-train.txt", fm_str4)
